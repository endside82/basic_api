...
(
    ...
    `receipt_id`    bigint    NOT NULL COMMENT '접수고유번호',
...
(
    `id`             bigint    NOT NULL AUTO_INCREMENT COMMENT '스케쥴고유번호',
    `stuff_id`       bigint    NOT NULL COMMENT '물건고유번호',
    `receipt_id`     bigint    NOT NULL COMMENT '접수고유번호',
    `date`           date               DEFAULT NULL COMMENT '스케쥴날짜',
    `schedule_type`  tinyint   NOT NULL COMMENT '스케쥴종류',
    `start_end_type` tinyint            DEFAULT NULL COMMENT '시작종료구분',
    `created_at`     timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성시각',
    `updated_at`     timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '변경시각',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_general_ci COMMENT ='스케쥴';